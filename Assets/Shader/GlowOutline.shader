			...
			half frag(v2f i) : COLOR
			{
				...
				for (int k = 0; k < NumberOfIterations; k += 1)
				{
					...
					ColorIntensityInRadius += tex2D( _MainTex, i.uvs.xy + 
						float2((k - NumberOfIterations / 2)*TX_x,0)).r / NumberOfIterations;
				}
			...
			half4 frag(v2f i) : COLOR
			{
				...
				for (int j = 0; j < NumberOfIterations; j += 1)
				{
					...
					ColorIntensityInRadius += tex2D(
						_GrabTexture, float2(i.uvs.x,1 - i.uvs.y) + 
						float2(0, (j - NumberOfIterations / 2)*TX_y)).r / NumberOfIterations;
				}
